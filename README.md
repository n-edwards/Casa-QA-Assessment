...
Written to show a range of passing and failing tests. Many tests are hard-coded in that they directly check for conditions in this test app that we don't expect to change, which is not always realistic in production.

Uses page object model pattern, a separate script that picks up HTML elements for use across test step scripts. This is not always necessary, especially for one-off test scripts like these, but is a good pattern to work with for learning reusable code.
...
### Test Structure  
Tests are numbered by loose priority. Script 05 is extra checks outside the initial scope of the assessment, to show some awareness of testing beyond UI/UX.
